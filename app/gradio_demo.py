...
class VisionDemo:
    ...
    def filter_predictions(self, predictions, query, task_type):
        ...
        if not target_objects:
            if parsed_task == "classification" or query.lower() in ["what is in this image", "classify this image"]:
                return predictions
        ...
        if task_type == VisionTaskType.OBJECT_DETECTION:
            for pred in predictions:
                ...
                if bbox:
                    ...
                    normalized_class = self.nlp_processor.normalize_object_name(class_name)
                    category = next((cat for cat, terms in self.nlp_processor.object_synonyms.items() 
                                  if normalized_class in terms), 'default')
            ...
            for i, pred in enumerate(predictions[:3]):
                ...
                label = f"{class_name}: {confidence:.2f}"
                y_pos = 30 + (i * 30)
                cv2.putText(img_array, label, (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 
                           1, (255, 255, 255), 3)
                cv2.putText(img_array, label, (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 
                           1, (0, 0, 0), 1)
        ...
        try:
            ...
            if result.task_type == VisionTaskType.IMAGE_CLASSIFICATION:
                ...
                filtered_preds = self.filter_predictions(
                    result.results['top_predictions'],
                    query,
                    result.task_type
                )
        ...
        with demo:
            ...
            with gr.Tabs():
                with gr.TabItem("Detection Examples"):
                    gr.Examples(
                        examples=[
                            ["examples/data/street.jpg", "OBJECT_DETECTION", "detect humans"],
                            ["examples/data/street.jpg", "OBJECT_DETECTION", "find vehicles"],
                            ["examples/data/living_room.jpg", "OBJECT_DETECTION", "detect all objects"]
                        ],
                ...
                with gr.TabItem("Classification Examples"):
                    gr.Examples(
                        examples=[
                            ["examples/data/dog.jpg", "IMAGE_CLASSIFICATION", "What breed is this dog?"],
                            ["examples/data/cat.jpg", "IMAGE_CLASSIFICATION", "Classify this image"]
                        ],
                        inputs=[input_image, task_type, query]
                    )
