...
def test_determine_task_type_detection(agent_router):
    ...
    agent_router.nlp_processor.parse_query = MagicMock(return_value=("detection", ["person"]))
    agent_router.agents[VisionTaskType.OBJECT_DETECTION] = MagicMock()
    agent_router.agents[VisionTaskType.OBJECT_DETECTION].model = MagicMock(names={0: "person"})
...
def test_determine_task_type_default(agent_router):
    ...
    agent_router.nlp_processor.parse_query = MagicMock(return_value=("unknown_task", []))
...
def test_process_request_no_agent(agent_router):
    ...
    with pytest.raises(ValueError, match="No agent registered for task type: VisionTaskType.OCR"):
...
def test_process_request_with_user_comment(agent_router):
    ...
    # Providing a mock image since VisionInput requires it
    mock_image = MagicMock()  # Replace with a valid image if needed
    vision_input = VisionInput(image=mock_image, task_type=None, user_comment="detect a person")
    agent_router.nlp_processor.parse_query = MagicMock(return_value=("detection", ["person"]))
...
def test_task_mapping(agent_router):
    ...
    assert agent_router.task_mapping["classification"] == VisionTaskType.IMAGE_CLASSIFICATION
...
def test_determine_task_type_with_invalid_target(agent_router):
    ...
    agent_router.nlp_processor.parse_query = MagicMock(return_value=("detection", ["unicorn"]))
    agent_router.agents[VisionTaskType.OBJECT_DETECTION] = MagicMock()
    agent_router.agents[VisionTaskType.OBJECT_DETECTION].model = MagicMock(names={0: "person"})
