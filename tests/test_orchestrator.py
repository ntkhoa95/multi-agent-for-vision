...
from vision_framework.core.types import VisionTaskType, VisionInput, VisionOutput, BatchDetectionResult, VideoDetectionResult
...
def test_initialize_agents(orchestrator):
    assert isinstance(orchestrator.router.agents[VisionTaskType.IMAGE_CLASSIFICATION], MobileNetClassificationAgent)
    assert isinstance(orchestrator.router.agents[VisionTaskType.OBJECT_DETECTION], YOLODetectionAgent)
...
def test_visualize_detections(orchestrator):
    image_path = "test_assets/images/street.jpg"
    detections = [
        {'class': 'person', 'bbox': [0, 0, 10, 10], 'confidence': 0.9},
        {'class': 'car', 'bbox': [10, 10, 20, 20], 'confidence': 0.8}
    ]
...
def test_process_image(mock_process_request, orchestrator):
    ...
    result = orchestrator.process_image("test_assets/images/street.jpg", "detect objects")
...
def test_process_batch(mock_process_batch, orchestrator):
    mock_process_batch.return_value = [
        BatchDetectionResult("test_assets/images/street.jpg", VisionOutput(
            task_type=VisionTaskType.OBJECT_DETECTION,
            results={'detections': [], 'num_detections': 0},
            confidence=0.0,
            processing_time=0.1
    ...
    results = orchestrator.process_batch(["test_assets/images/street.jpg"], VisionTaskType.OBJECT_DETECTION)
...
def test_process_video(mock_process_video, orchestrator):
    mock_process_video.return_value = VideoDetectionResult(
        video_path="test_videos/crosswalk.avi",
        frames_results=[],
        fps=30.0,
        total_time=10.0
    )
    result = orchestrator.process_video("test_videos/crosswalk.avi")
    assert result.video_path == "test_videos/crosswalk.avi"
...
def test_process_batch_no_agent(orchestrator):
    with patch.object(VisionTaskType, 'IMAGE_SEGMENTATION', create=True):
        with pytest.raises(ValueError):
            orchestrator.process_batch(["test_assets/images/street.jpg"], VisionTaskType.IMAGE_SEGMENTATION)

def test_process_video_no_agent(orchestrator):
    with patch.object(VisionTaskType, 'IMAGE_SEGMENTATION', create=True):
        with patch.object(AgentRouter, 'determine_task_type', return_value=(VisionTaskType.IMAGE_SEGMENTATION, None)):
            with pytest.raises(ValueError):
                orchestrator.process_video("test_videos/crosswalk.avi", user_comment="segment video")

def test_process_video_no_support(orchestrator):
    with patch.object(AgentRouter, 'determine_task_type', return_value=(VisionTaskType.IMAGE_CLASSIFICATION, None)):
        with pytest.raises(ValueError):
            orchestrator.process_video("test_videos/crosswalk.avi", user_comment="classify video")
