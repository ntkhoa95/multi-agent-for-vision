...
class MobileNetClassificationAgent(BaseVisionAgent):
    ...
    def load_model(self):
        ...
        model = timm.create_model(
            self.config["MODEL_NAME"], pretrained=self.config["MODEL_PRETRAINED"]
        )
    ...
    def load_imagenet_labels(self):
        ...
        labels_path = Path("examples/imagenet_labels.json")
    ...
    def process(self, vision_input: VisionInput) -> VisionOutput:
        ...
        # with torch.amp.autocast(enabled=self.config.get('USE_FP16', False)):
        with torch.no_grad():
        ...
        results = {
            "top_predictions": [
                {"class": self.labels[idx.item()], "confidence": prob.item()}
